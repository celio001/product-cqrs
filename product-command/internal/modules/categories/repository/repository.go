...
type CategoriesInterface interface {
	...
	WithTx(tx pgx.Tx) CategoriesInterface
	BeginTx(context.Context) (pgx.Tx, error)
}

func NewCategoriesRepo(PgPool *pgxpool.Pool, tx *database.Queries) CategoriesInterface {
...
func (r *categoriesRepo) WithTx(tx pgx.Tx) CategoriesInterface {
	return &categoriesRepo{
		PgPool: r.PgPool,
		Tx:     r.Tx.WithTx(tx),
	}
...
func (r *categoriesRepo) BeginTx(ctx context.Context) (pgx.Tx, error) {
...
func (c *categoriesRepo) GetCategoryByID(ctx context.Context, id uuid.UUID) (categories.Categories, error) {
	...
	err := c.Tx.DB.QueryRow(ctx, query, id).Scan(&category.ID, &category.Name)
	if err != nil {
...
func (c *categoriesRepo) CreateCategory(ctx context.Context, categorie categories.Categories) (categories.Categories, error) {
	...
	err := c.Tx.DB.QueryRow(ctx, query, categorie.Name).Scan(&categorie.ID, &categorie.ParentID)
	if err != nil {
		return categories.Categories{}, err
	}
...
func (c *categoriesRepo) SoftDeleteCategory(ctx context.Context, id uuid.UUID) error {
	...
	result, err := c.Tx.DB.Exec(ctx, query, id)
	if err != nil {
		return err
	}
